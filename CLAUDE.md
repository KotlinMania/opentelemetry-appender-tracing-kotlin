# Claude Code Project Instructions — opentelemetry-appender-tracing-kotlin

## Project Overview

This is **opentelemetry-appender-tracing-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`opentelemetry-appender-tracing`](https://crates.io/crates/opentelemetry-appender-tracing).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/opentelemetry-appender-tracing/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/opentelemetry-appender-tracing>
