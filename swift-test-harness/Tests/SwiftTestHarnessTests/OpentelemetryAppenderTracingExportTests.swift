...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `OpentelemetryAppenderTracing.swiftmodule/`
...
//      `import OpentelemetryAppenderTracing` above would fail at compile time.
//
//   2. The static archive `libOpentelemetryAppenderTracing.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "OpentelemetryAppenderTracing" }` and
//      `flattenPackage = "io.github.kotlinmania.opentelemetryappendertracing"` configuration in
...
//      via the `OpentelemetryAppenderTracingLibrary` product.
//
...
final class OpentelemetryAppenderTracingExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "OpentelemetryAppenderTracing swift module imported cleanly")
    }
}
